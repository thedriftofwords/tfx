...
"""Platform config for docker run."""
...
class DockerPlatformConfig(base_platform_config.BasePlatformConfig):
  ...
  Attributes:
    docker_server_url: URL to the Docker server. For example,
      `unix:///var/run/docker.sock` or `tcp://127.0.0.1:1234`. Uses environment
        viarable to intialize the docker client if this parameter is not set.
        Default: `None`.

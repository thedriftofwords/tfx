...
from tfx.orchestration.config import docker_platform_config
from tfx.orchestration.launcher import docker_component_launcher
from tfx.types import component_spec


class HelloWorldSpec(component_spec.ComponentSpec):
  INPUTS = {}
  OUTPUTS = {}
  PARAMETERS = {
...
class HelloWorldComponent(base_component.BaseComponent):

  SPEC_CLASS = HelloWorldSpec
  ...
  def __init__(self, name):
    super(HelloWorldComponent, self).__init__(HelloWorldSpec(name=name))
...
def _create_pipeline(
  ...
  hello_world = HelloWorldComponent(name=name)
...
class DockerComponentLauncherE2eTest(tf.test.TestCase):
  ...
  def testDockerComponentLauncherInBeam(self):
    beam_dag_runner.BeamDagRunner.SUPPORTED_LAUNCHER_CLASSES.append(
        docker_component_launcher.DockerComponentLauncher)

    beam_dag_runner.BeamDagRunner(platform_configs={
        'HelloWorldComponent': [docker_platform_config.DockerPlatformConfig()]
    }).run(
        _create_pipeline(
    ...
    with metadata.Metadata(metadata_config) as m:
      ...
      print(m.store.get_executions()[0])

...
class DockerComponentLauncher(base_component_launcher.BaseComponentLauncher):
  ...
  def can_launch(
    ...
    if platform_config and not isinstance(
        platform_config, docker_platform_config.DockerPlatformConfig):
      return False
  ...
  def _run_executor(self, execution_id: int,
    ...
    if self._platform_config:
      docker_config = cast(docker_platform_config.DockerPlatformConfig,
                           self._platform_config)
    else:
      docker_config = docker_platform_config.DockerPlatformConfig()
    ...
    tf.logging.info('Container spec: %s' % vars(executor_container_spec))
    tf.logging.info('Docker config: %s' % vars(docker_config))

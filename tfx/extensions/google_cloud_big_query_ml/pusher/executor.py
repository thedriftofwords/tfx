...
from tfx.extensions.google_cloud_bigquery_ml import runner
...
class Executor(tfx_pusher_executor.Executor):
  """Deploy a model to Big Query for serving."""
  ...
  def Do(self, input_dict: Dict[Text, List[types.Artifact]],
    ...
    bigquery_serving_args = custom_config['bigquery_serving_args']
    ...
    if bigquery_serving_args is None:
      model_push.set_int_custom_property('pushed', 0)
      absl.logging.info(
          'Big Query ML configuration was not provided for model %s',
          model_blessing_uri)
      return

    runner.deploy_model_for_bigquery_serving(model_path, model_version,
